...
mcc_tab = read.csv("./A6_concat-genome_traits_2026-03-10_MCC.csv", head=T)


# Step 3 - Plot the CSV and save the plot as a png to the output_plots directory (Chat GPT helped a lot with this)
mcc_file <- "./A6_concat-genome_traits_2026-03-10_MCC.csv"
...
#  polygons = suppressWarnings(spreadGraphic2(localTreesDirectory, nberOfExtractionFiles, prob, startDatum, precision))
  ...
  colour_scale = colorRampPalette(brewer.pal(11,"RdYlGn"))(141)[21:121] # This line provides a gradient of 100 colors from red > yellow > green
...
#  }
  ...
  pdf(file = file.path("output_plots", output_file), width = 6, height = 6.3)
  par(mar=c(0,0,0,0), oma=c(1.2,3.5,1,0), mgp=c(0,0.4,0), lwd=0.2, bty="o")
  plot(template_raster, col="lightgrey", box=F, axes=F, colNA="lightcyan", legend=F)
...
#    points(k[i,"startLon"], k[i,"startLat"], pch=16, col="gray10", cex=0.13) # But this line ensure no other starting points are colored
    ...
    x_jit <- jitter(k[i, "endLon"]) 
    y_jit <- jitter(k[i, "endLat"])
    ...
    #            arr.width=0, lwd=0.2, lty=1, lcol="gray32", arr.col=NA, arr.pos=FALSE, curve=0.1, dr=NA, endhead=F)
    curvedarrow(cbind(k[i,"startLon"],k[i,"startLat"]), cbind(x_jit,y_jit), arr.length=0,
                arr.width=0, lwd=0.2, lty=1, lcol="gray32", arr.col=NA, arr.pos=FALSE, curve=0.1, dr=NA, endhead=F)
    ...
    #points(k[i,"endLon"], k[i,"endLat"], pch=16, col=endYears_colours[i], cex=0.3) # These next two lines color the ending points, this one colors by time
    ...
    #points(k[i,"endLon"], k[i,"endLat"], pch=ext_node_shapes[(k[i, "isnode"])], col="gray10", cex=0.3) # This line of code provides an dark outline for each ending point
    
    points(x_jit, y_jit, pch=int_node_shapes[(k[i,"isnode"])], col = host_colors[(k[i, "endHost"])], cex = 0.3) # This line jitters the colored internal part of ending points
    points(x_jit, y_jit, pch=ext_node_shapes[(k[i,"isnode"])], col = "gray10", cex = 0.3) # This line jitters the external outline of ending points
